...
| **Seguridad**            | Intento de acceso no autorizado a lista de reseñas de otro usuario |   Usuario no autenticado           | Módulo de estadísticas y reseñas    | Operación normal     | El sistema deniega el acceso y registra el intento en los logs de seguridad | El acceso no autorizado es completamente bloqueado y el intento es registrado con detalles de IP, timestamp y acción en los logs de seguridad |
| **Rendimiento**        | Aumento significativo en la demanda de acceso a lista de reseñas de un juego | Usuarios registrados  | Servidor de base de datos de Reseñas y Calificaciones | Periodo de alta demanda      | El sistema genera y entrega la lista sin afectar el rendimiento general | El sistema debe soportar hasta 10,000 consultas concurrentes sin pérdida de rendimiento perceptible ni fallos en la entraga de las listas |
| **Disponibilidad**       | Acceso simultáneo a lista de reseñas por juego | Usuarios autenticados | Módulo de estadísticas, reseñas y calificaciones | Durante el uso normal      | El sistema debe responder a todas las solicitudes sin interrupciones | El sistema debe mantener un 99.95% de disponibilidad |
| **Usabilidad**           | Envío de reseña que excede el límite de caracteres permitidos | Usuario autenticado             | Sistema de validación del Módulo de Reseñas    | Condiciones normales de uso      | El sistema muestra un mensaje de error claro e inmediato explicando el límite | El mensaje de error debe ser fácilmente comprensible, orientado al usuario, y permitir la corrección de manera sencilla y directa |
| **Fiabilidad**           | Error al guardar una calificación debido a fallo en la conexión de red | Usuario autenticado             | Módulo de Reseñas y Calificaciones    | Condiciones de fallo intermitente de red      | El sistema debe reintentar automáticamente y notificar al usuario si el fallo persiste | El sistema debe reintentar la operación automáticamente al menos 3 veces, y notificar al usuario solo si el error no se puede resolver tras los intentos |
